...
def main():
    ...
    while time_s != 0:
        speedy = int(input("Enter a speed (0 to 900 dps): "))
        if speedy == 0:
            break
        distance = int(input("Distance to Travel (inches): "))
        if distance == 0:
            break
        left_motor.run_to_rel_pos(position_sp=distance*90,speed_sp=speedy,stop_action=ev3.Motor.STOP_ACTION_BRAKE)
        right_motor.run_to_rel_pos(position_sp=distance*90,speed_sp=speedy,stop_action=ev3.Motor.STOP_ACTION_BRAKE)
