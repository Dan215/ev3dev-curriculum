...
Much like we did in Dan of the motors unit, later we will show you different (better) ways to use buttons.
...
def main():
    ...
    while True:
        ...
        btn = ev3.Button()
        colors = [ev3.Leds.GREEN, ev3.Leds.RED, ev3.Leds.AMBER, ev3.Leds.BLACK]
        number = 0
        while True:
            if btn.up:
                print('up')
            ...
            if btn.right:
                ...
                ev3.Leds.set_color(ev3.Leds.LEFT, ev3.Leds.BLACK)
            if btn.down:
                ev3.Leds.set_color(ev3.Leds.LEFT, colors[number % 4])
                ev3.Leds.set_color(ev3.Leds.RIGHT, colors[number % 4])
                while True:
                    if not btn.down:
                        number += 1
                        break
